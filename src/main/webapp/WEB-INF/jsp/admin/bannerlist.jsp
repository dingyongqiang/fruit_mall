            ...
            var tableIns = table.render({
                ...
                , cols: [[
                   ...
                   ,{ field:'bannerState', title: '轮播图状态', width: 112, align: 'center',templet: function(d){
                	   if(d.bannerState==1){
                        	  return "<input type='checkbox' lay-filter='switchTest' name='"+d.bannerState+"' lay-skin='switch' lay-text='禁用|正常'><input type='hidden' id='kaigID' value='"+d.bannerId+"' />"
                     	 }else{                    	 
                       	  return "<input type='checkbox' lay-filter='switchTest' name='"+d.bannerState+"' lay-skin='switch' lay-text='禁用|正常' checked><input type='hidden' id='kaigID' value='"+d.bannerId+"' />"
                     	 }	
                	 }
			...
			form.on('switch(switchTest)',function(data){
				...
				if(data.elem.checked ==true){
					$.ajax({
						...
						data:"bannerId="+id+"&bannerState="+0,
					...
					$.ajax({
						type:"post",
						url:"banner/changeBannerState",
						data:"bannerId="+id+"&bannerState="+1,
						success:function(data){
							if(data=="success"){
